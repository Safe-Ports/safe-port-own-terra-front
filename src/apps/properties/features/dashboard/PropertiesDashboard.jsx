...
const { open: HiArrowRight } = PROPERTY_ACTION_ICONS;
const { community: HiBuildingOffice2, rent: HiKey, sale: HiTag, unit: HiHomeModern, portfolio: HiOutlineSquares2X2, owner: HiUserGroup } = PROPERTY_ENTITY_ICONS;

const paths = [
  { key:"communities", number:"01", label:"Comunidades", caption:"Espacios y operación compartida", description:"Coordina condominios, privadas, plazas, cabañas u hoteles con sus unidades, accesos y servicios.", action:"Abrir comunidades", icon:HiBuildingOffice2, tone:"community", status:"Disponible", journey:["Configura","Conecta personas","Opera servicios","Atiende"] },
...
function PropertiesDashboard(){
  ...
  const {properties,units,communities,propertiesLoading}=usePropertiesData();
  ...
  const openPath=(key)=>{
    ...
    return navigate("/properties/comunidades");
  };
  return <EcoLayout active="properties" title="OwnTerra Properties" subtitle={`${organizationName} · Todo en su lugar`}>
    <main className="properties-dashboard">
      {!propertiesLoading&&!communities.length?<section className="properties-setup-callout"><span><HiBuildingOffice2/></span><div><small>Configuración inicial</small><h2>Crea tu primera comunidad</h2><p>Registra el lugar, define su estructura y deja lista la operación en tres pasos guiados.</p></div><button type="button" onClick={()=>navigate("/properties/guia")}>Comenzar configuración <HiArrowRight/></button></section>:null}
      <section className="properties-hero"><div><span className="properties-eyebrow"><HiHomeModern/> Resumen operativo</span><h1>Buen día, {firstName}.</h1><p>Revisa el estado del portafolio y entra al área que requiere tu atención.</p>{canUseFeature("properties.units.read")?<button className="properties-monitoring-cta" type="button" onClick={()=>navigate("/properties/monitoreo")}><HiHomeModern/> Monitoreo por unidad <HiArrowRight/></button>:null}<button className="properties-guide-cta" type="button" onClick={()=>navigate("/properties/guia")}>Guía de configuración <HiArrowRight/></button></div><div className="properties-pulse" aria-label="Resumen del portafolio"><div><strong>{activeProperties}</strong><span>Propiedades</span></div><div><strong>{occupancy}</strong><span>Ocupación</span></div><div><strong>{attentionUnits}</strong><span>Por atender</span></div></div></section>
