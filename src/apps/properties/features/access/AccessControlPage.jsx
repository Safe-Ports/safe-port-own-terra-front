...
import PackageDeskPanel from "./PackageDeskPanel";
...
function AccessControlPage(){
  ...
  return <EcoLayout active="properties" title="OwnTerra Properties" subtitle="Operación · Accesos"><main className="access-page">
    <header className="access-heading"><button type="button" onClick={()=>navigate("/properties/comunidades/operacion")}><HiArrowLeft/> Operación comunitaria</button><div><span>Recepción y paquetería</span><h1>Accesos sin fricción.</h1><p>La caseta recibe instrucciones claras para visitas, proveedores y entregas.</p></div><button type="button" onClick={()=>setCreating(true)}><HiPlus/> Crear pase</button></header>
    <PackageDeskPanel/>
    <aside className="access-prototype-note"><span>▲</span><span><b>Lo de abajo es prototipo.</b> Los pases de visita no existen en el backend: lo que registres aquí se pierde al recargar. La paquetería de arriba sí se guarda.</span></aside>
