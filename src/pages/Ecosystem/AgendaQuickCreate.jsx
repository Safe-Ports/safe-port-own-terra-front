...
import { CREATABLE_APPS, TYPE_KEYS, buildAppointmentBody } from "./agendaShared";

const POPOVER_WIDTH = 300;
const GAP = 8;

function formatSlotLabel(date, time, localeTag) {
  const label = new Date(`${date}T12:00:00`).toLocaleDateString(localeTag, {
    weekday: "short",
    day: "numeric",
    month: "short",
  });
...
function AgendaQuickCreate({ date, time, anchorRect, onClose, onSubmit, onMore }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const body = buildAppointmentBody({ title, date, time, type, app, client: "", context: "", owner: "" }, t);
      await onSubmit(body);
      onClose();
    } finally {
      setSubmitting(false);
  ...
  return (
    <div ref={boxRef} className="ag-quickcreate" style={{ ...style, width: POPOVER_WIDTH }} onMouseDown={(e) => e.stopPropagation()}>
      <form onSubmit={handleSubmit}>
        <div className="ag-quickcreate-slot">{formatSlotLabel(date, time, localeTag)}</div>
        <input
          ref={titleRef}
          className="ag-quickcreate-title"
          value={title}
          onChange={(e) => setTitle(e.target.value)}
          placeholder={t("agenda.addTitle")}
        />
        ...
        <div className="ag-quickcreate-foot">
          <button type="button" className="ag-quickcreate-more" onClick={() => onMore(date, time)}>{t("agenda.moreOptions")}</button>
          <button type="submit" className="ag-primary" disabled={submitting}>
            {submitting ? t("agenda.saving") : t("agenda.save")}
          </button>
        </div>
      </form>
    </div>
