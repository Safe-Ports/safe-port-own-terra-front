...
export function AppProvider({ children }) {
  ...
  const saveContract = async (payload) => {
    try {
      if (payload.id) {
        await contractService.update(payload.id, { notes: payload.notes });
      } else {
        await contractService.create({
          client_id: payload.client_id ?? payload.clientId,
          lot_id: payload.lot_id ?? payload.lotId ?? (payload.lot ? Number(payload.lot) : undefined),
          type: payload.type,
  ...
  const saveDocument = async (payload, file) => {
    ...
    try {
      await documentService.upload(file, {
        ...
        entityType: payload.linkType ? toBackendEntityType(payload.linkType) : undefined,
  ...
  const downloadDocument = (id) => {
    window.open(documentService.downloadUrl(id), "_blank");
  };
  ...
  // ── Fraccionamientos ──────────────────────────────────────────────────────
  const saveFrac = async ({ name, sections }) => {
    try {
